...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signUp = async (email: string, password: string, role: 'worker'|'employer', fullName: string) => {
    ...
    if (!error && data.user) {
      await supabase.from('profiles').upsert({ id: data.user.id, role, full_name: fullName, skills: [], certificates_urls: [], availability: 'available', avg_rating: 0, review_count: 0 });
    }
    return { error };
  };
